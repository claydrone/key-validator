...
def server_config_info() -> Chain:
    ...
    for url in url_list:
        ...
        data = r.json()
        server_config.append(Chain(data['environment'], network=data['network'],
...
def test_mumbai_config(chain_info, server_config_info):
    for server in server_config_info:
        ...
        assert chain_info['environments'][env]['network'] == server.network
        assert chain_info['environments'][env]['payment_contract_address'] == server.payment_contract_address
        assert chain_info['environments'][env]['signer_contract_address'] == server.signer_contract_address
